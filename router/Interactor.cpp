...
Interactor::Interactor() {
	memset(this->input, 0, _INTERACTOR_H_INPUT_LEN);
	memset(this->IPs, 0, sizeof(IPAddr_t)*_INTERACTOR_H_MAX_IP_NUM);
	this->router = NULL;
	this->handle = NULL;
	if (this->selDev() != NULL) {
		// 设置过滤器
...
void Interactor::saveIPs(pcap_if_t * curdev){
	int i = 0;
	for (pcap_addr_t* a = curdev->addresses; a != NULL; a = a->next) {	//将扫描的网卡IP和MASK存进来
...
void Interactor::initRouter(Router*){
	for (int i = 0; i < _INTERACTOR_H_MAX_IP_NUM && this->IPs[i] != 0; i = i + 1) {
		Router_Record record;	//添加自己IP的默认路由
		MACAddr_t MAC;
...
void Interactor::printHelp() {
	printf("Usage: router/r [options] [target] ...\n");
	printf("Options:\n");

	printf("\tadd\t\tAdd a record to router table. ");
	printf("router add [DesIP] [NetMask] [NextHop]\n");

	printf("\tdel\t\tDelete a record from router table. ");
...
char * Interactor::getInput() {
	memset(this->input, 0, _INTERACTOR_H_INPUT_LEN);
	if (fgets(this->input, _INTERACTOR_H_INPUT_LEN - 2, stdin) == NULL){
		return NULL;
	}
...
int Interactor::parseInput(char * _input_){
	...
	if (strcmp(argv[0], "router") != 0 && strcmp(argv[0], "r") != 0) {
		return 0;
	}
...
char** Interactor::split(int* argc, char* _input_){	//按空格劈开，子串指针存tempargv里
	memset(this->tempargv,'\0',256);
	char last = ' ';
	*argc = 0;
	tempargv[0] = (char *)_input_;
	...
	for (int i = 0; i < len; i = i + 1) {
		...
		if (_input_[i] == ' ') {
			_input_[i] = '\0';
			tempargv[*argc] = (char*)(_input_ + i + 1);
		}
		last = _input_[i];
	}
	return tempargv;
