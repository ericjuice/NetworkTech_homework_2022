...
#define _ROUTER_H_TABLE_LEN 32	   // 路由表长度
#define _ROUTER_H_MACTABLE_LEN 128 // MAC映射表长度
#define _ROUTER_H_LOG_LEN 512	   // 日志记录长度
#define _ROUTER_H_MAX_BIND_IP 10   // 绑定的IP
...
{
public:
	struct IP_MAC_Record macTable[_ROUTER_H_MACTABLE_LEN];						   // MAC和IP映射表
	struct MyRouter_Record table[_ROUTER_H_TABLE_LEN];							   // 路由表
	IPAddr_t bindIPs[_ROUTER_H_MAX_BIND_IP];									   // 绑定的IP
	MACAddr_t MAC;																   // MAC地址
